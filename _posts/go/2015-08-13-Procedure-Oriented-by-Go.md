...
continue是跳过本次循环，循环可能还未结束；break和continue还可以跟着标号，用来跳到多重循环中
...
Go语言中的函数值传递与Java语言中一样，均是指当传递一个参数值到被调用函数里面时，实际上传了这个

值的一份copy,当在被调用函数中修改参数值的时候，调用函数中实参不会发生变化。


指针传递是指将变量存放在内存中的地址&x传入函数，若函数中改变了该地址的值，则意味着原传入函数的
...
在Go语言中函数也是一种变量，可以通过type定义它，它的类型是所有拥有相同的参数，相同的返回值。
...
入，若一个包被多个同时导入，则只会被导入一次。当一个包被导入时，如果该包还导入了其他包，那

么会先将其他包导入进来，然后再对这些包中的包级常量和变量进行初始化，接着执行init函数，依次

类推，等所有导入的包被加载完毕，就开始对main包中的包级常量和变量进行初始化，然后执行main包
